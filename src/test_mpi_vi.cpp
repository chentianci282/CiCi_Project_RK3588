...
void init_venc_cfg(TEST_VI_CTX_S *ctx, RK_U32 u32Ch, RK_CODEC_ID_E enType) {
    ...
    // 注意：这里先用 width/height，后续在获取到实际 VI 帧后会更新为实际的 virWidth/virHeight
...
// 更新 VENC 配置为实际 VI 帧参数（用于排障，实际可能不支持动态修改）
static void update_venc_cfg_with_actual_frame(TEST_VI_CTX_S *ctx, RK_U32 u32Ch, VIDEO_FRAME_INFO_S *pstFrame) {
    RK_U32 u32VirWidth = pstFrame->stVFrame.u32VirWidth;
    RK_U32 u32VirHeight = pstFrame->stVFrame.u32VirHeight;
    RK_U32 u32ActualWidth = pstFrame->stVFrame.u32Width;
    RK_U32 u32ActualHeight = pstFrame->stVFrame.u32Height;
    RK_U32 u32BufSize = RK_MPI_MB_GetLength(pstFrame->stVFrame.pMbBlk);
    
    RK_LOGE("[排障] ========== VI实际帧参数 ==========");
    RK_LOGE("[排障] Width=%u, Height=%u", u32ActualWidth, u32ActualHeight);
    RK_LOGE("[排障] VirWidth=%u, VirHeight=%u", u32VirWidth, u32VirHeight);
    RK_LOGE("[排障] BufSize=%u 字节 (%.2f MiB)", u32BufSize, (float)u32BufSize / (1024*1024));
    RK_LOGE("[排障] PixelFormat=%d (0=NV12, 10=YUYV, ...)", pstFrame->stVFrame.enPixelFormat);
    RK_LOGE("[排障] CompressMode=%d", pstFrame->stVFrame.enCompressMode);
    
    // 打印 stride 信息（如果存在）
    // 注意：stride 信息可能在不同版本的结构体中位置不同，这里先注释掉，避免编译错误
    // 如果需要 stride 信息，可以通过其他方式获取或根据 VirWidth 计算
    RK_LOGE("[排障] 注: Stride通常等于VirWidth（对于Y平面），可通过VirWidth估算");
    
    // 计算期望的 BufSize（基于 VirWidth * VirHeight）
    RK_U32 u32ExpectedBufSize = 0;
    if (pstFrame->stVFrame.enPixelFormat == RK_FMT_YUV420SP) {
        u32ExpectedBufSize = u32VirWidth * u32VirHeight * 3 / 2;  // NV12
    } else if (pstFrame->stVFrame.enPixelFormat == RK_FMT_YUV422_YUYV) {
        u32ExpectedBufSize = u32VirWidth * u32VirHeight * 2;  // YUYV
    } else {
        u32ExpectedBufSize = u32VirWidth * u32VirHeight * 3 / 2;  // 默认按NV12估算
    }
    
    RK_LOGE("[排障] 期望BufSize(基于VirWidth*VirHeight): %u 字节", u32ExpectedBufSize);
    if (u32BufSize != u32ExpectedBufSize) {
        RK_LOGE("[排障] ⚠️  实际BufSize != 期望BufSize，差值=%d 字节", (int)(u32BufSize - u32ExpectedBufSize));
    }
    
    // 检查是否需要更新 VENC 配置
    if (u32VirWidth != (RK_U32)ctx->width || u32VirHeight != (RK_U32)ctx->height) {
        RK_LOGE("[排障] ⚠️  ⚠️  ⚠️  关键发现: VI实际VirWidth/VirHeight != 配置值！");
        RK_LOGE("[排障] ⚠️  配置: width=%d, height=%d", ctx->width, ctx->height);
        RK_LOGE("[排障] ⚠️  实际: VirWidth=%u, VirHeight=%u", u32VirWidth, u32VirHeight);
        RK_LOGE("[排障] ⚠️  这很可能导致VENC配置的bufsize不匹配，编码链路异常！");
        RK_LOGE("[排障] ⚠️  建议: VENC应配置 VirWidth=%u, VirHeight=%u, BufSize>=%u",
                u32VirWidth, u32VirHeight, u32BufSize);
        RK_LOGE("[排障] ⚠️  解决方案: 在创建VENC之前先获取一帧VI，然后用实际参数创建VENC");
    } else {
        RK_LOGE("[排障] ✓ VirWidth/VirHeight 与配置值匹配");
    }
    RK_LOGE("[排障] ========================================");
}
...
static RK_S32 test_vi_bind_vpss_venc_loop(TEST_VI_CTX_S *ctx) {
    ...
    RK_BOOL bViFrameGot = RK_FALSE;  // 标记是否已获取到 VI 帧用于诊断
    RK_S32 viFrameWaitTime = 33;
    ...
    // [排障步骤1] 如果 depth > 0，在 bind 之前先获取一帧 VI 帧，获取实际参数
    if (ctx->stChnAttr.u32Depth > 0) {
        RK_LOGE("[排障] [VI->VPSS->VENC] depth=%d > 0，先获取一帧 VI 帧来获取实际参数", ctx->stChnAttr.u32Depth);
        s32Ret = RK_MPI_VI_GetChnFrame(ctx->pipeId, ctx->channelId, &ctx->stViFrame, viFrameWaitTime);
        if (s32Ret == RK_SUCCESS) {
            RK_LOGE("[排障] ✓ VI 成功产出第一帧！");
            update_venc_cfg_with_actual_frame(ctx, 0, &ctx->stViFrame);
            s32Ret = RK_MPI_VI_ReleaseChnFrame(ctx->pipeId, ctx->channelId, &ctx->stViFrame);
            if (s32Ret != RK_SUCCESS) {
                RK_LOGE("[排障] RK_MPI_VI_ReleaseChnFrame fail %x", s32Ret);
            }
            bViFrameGot = RK_TRUE;
        } else {
            RK_LOGE("[排障] ⚠️  VI 获取第一帧失败: 0x%x", s32Ret);
        }
    } else {
        RK_LOGE("[排障] [VI->VPSS->VENC] depth=%d，无法从用户侧获取 VI 帧（建议加 -D 1 或 -D 3）", ctx->stChnAttr.u32Depth);
    }
    ...
    while (loopCount < ctx->loopCountSet) {
        for (i = 0; i < u32DstCount; i++) {
            ...
            // [排障步骤2] 如果 depth > 0，同时获取 VI 帧和 VENC 流进行对比
            if (ctx->stChnAttr.u32Depth > 0) {
                s32Ret = RK_MPI_VI_GetChnFrame(ctx->pipeId, ctx->channelId, &ctx->stViFrame, viFrameWaitTime);
                if (s32Ret == RK_SUCCESS) {
                    if (loopCount == 0 || loopCount % 30 == 0) {  // 每30帧打印一次
                        RK_LOGE("[排障] [VI->VPSS->VENC] [VI帧] loopCount=%d, Width=%u, Height=%u, VirWidth=%u, VirHeight=%u, "
                                "BufSize=%u, PTS=%lld, Seq=%d",
                                loopCount,
                    ...
                    if (loopCount % 30 == 0) {
                        RK_LOGE("[排障] [VI->VPSS->VENC] ⚠️  VI GetChnFrame 失败: 0x%x (loopCount=%d)", s32Ret, loopCount);
                    }
                }
            }

            s32Ret = RK_MPI_VENC_GetStream(ctx->stVencCfg[i].s32ChnId, &ctx->stFrame[i], 1000);  // 改为1000ms超时
            if (s32Ret == RK_SUCCESS) {
                ...
                if (loopCount == 0 || loopCount % 30 == 0) {
                    RK_U64 nowUs = TEST_COMM_GetNowUs();
                    RK_LOGE("[排障] [VI->VPSS->VENC] [VENC流] loopCount=%d, chn=%d, seq=%d, len=%d, pts=%lld, delay=%lldus",
                            loopCount, i, ctx->stFrame[i].u32Seq, ctx->stFrame[i].pstPack->u32Len,
                            ctx->stFrame[i].pstPack->u64PTS, nowUs - ctx->stFrame[i].pstPack->u64PTS);
                }
                ...
                if (loopCount % 10 == 0) {  // 每10次失败打印一次，避免刷屏
                    RK_LOGE("[排障] [VI->VPSS->VENC] ⚠️  VENC GetStream 失败: 0x%x (loopCount=%d)", s32Ret, loopCount);
                    if (ctx->stChnAttr.u32Depth > 0) {
                        RK_LOGE("[排障] 诊断: VI能GetFrame但VENC不能GetStream → 可能是VPSS/VENC格式/内存布局不匹配");
                    } else {
                        RK_LOGE("[排障] 诊断: 建议加 -D 1 验证 VI 是否真正出帧");
                    }
                }
            }
        }
...
static RK_S32 test_vi_bind_venc_loop(TEST_VI_CTX_S *ctx) {
    ...
    // [排障步骤1] 如果 depth > 0，在 bind 之前先获取一帧 VI 帧，获取实际参数
    if (ctx->stChnAttr.u32Depth > 0) {
        RK_LOGE("[排障] depth=%d > 0，先获取一帧 VI 帧来获取实际参数", ctx->stChnAttr.u32Depth);
        s32Ret = RK_MPI_VI_GetChnFrame(ctx->pipeId, ctx->channelId, &ctx->stViFrame, viFrameWaitTime);
        if (s32Ret == RK_SUCCESS) {
            RK_LOGE("[排障] ✓ VI 成功产出第一帧！");
            update_venc_cfg_with_actual_frame(ctx, 0, &ctx->stViFrame);
        ...
        RK_LOGE("[排障] depth=%d，无法从用户侧获取 VI 帧（建议加 -D 1 或 -D 3）", ctx->stChnAttr.u32Depth);
    }
    ...
    while (loopCount < ctx->loopCountSet) {
        for (i = 0; i < u32DstCount; i++) {
            ...
            if (ctx->stChnAttr.u32Depth > 0) {
                ...
                if (s32Ret == RK_SUCCESS) {
                    if (loopCount == 0 || loopCount % 30 == 0) {  // 每30帧打印一次
                        RK_LOGE("[排障] [VI帧] loopCount=%d, Width=%u, Height=%u, VirWidth=%u, VirHeight=%u, "
                                "BufSize=%u, PTS=%lld, Seq=%d",
                                loopCount,
                                ctx->stViFrame.stVFrame.u32Width,
                                ctx->stViFrame.stVFrame.u32Height,
                    ...
                    if (loopCount % 30 == 0) {
                        RK_LOGE("[排障] ⚠️  VI GetChnFrame 失败: 0x%x (loopCount=%d)", s32Ret, loopCount);
                    }
                }
            }

            // [排障步骤3] 查询 VENC 状态（输入/输出队列）
            // 注意：如果 API 不存在，这里会编译失败，需要根据实际 API 调整
            // VENC_CHN_STATUS_S stVencStatus;
            // s32Ret = RK_MPI_VENC_QueryStatus(ctx->stVencCfg[i].s32ChnId, &stVencStatus);
            // if (s32Ret == RK_SUCCESS && (loopCount == 0 || loopCount % 30 == 0)) {
            //     RK_LOGE("[排障] [VENC状态] chn=%d, 输入队列=%d, 输出队列=%d",
            //             i, stVencStatus.u32LeftPics, stVencStatus.u32LeftPacks);
            // }
            ...
            if (s32Ret == RK_SUCCESS) {
                ...
                if (loopCount == 0 || loopCount % 30 == 0) {
                    RK_LOGE("[排障] [VENC流] loopCount=%d, chn=%d, seq=%d, len=%d, pts=%lld, delay=%lldus",
                            loopCount, i, ctx->stFrame[i].u32Seq, ctx->stFrame[i].pstPack->u32Len,
                            ctx->stFrame[i].pstPack->u64PTS, nowUs - ctx->stFrame[i].pstPack->u64PTS);
                }
                ...
                if (loopCount % 10 == 0) {  // 每10次失败打印一次，避免刷屏
                    RK_LOGE("[排障] ⚠️  VENC GetStream 失败: 0x%x (loopCount=%d)", s32Ret, loopCount);
                    if (ctx->stChnAttr.u32Depth > 0) {
                        RK_LOGE("[排障] 诊断: VI能GetFrame但VENC不能GetStream → 可能是格式/内存布局不匹配");
                    } else {
                        RK_LOGE("[排障] 诊断: 建议加 -D 1 验证 VI 是否真正出帧");
                    }
                }
