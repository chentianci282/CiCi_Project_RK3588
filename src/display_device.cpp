...
int DisplayDevice::init(const std::string& device_path, uint32_t connector_id) {
    ...
    if (m_connector->count_modes > 0) {
        // 查找720p模式（1280x720），如果没有则使用第一个模式
        m_selected_mode_index = 0;
        ...
        for (int i = 0; i < m_connector->count_modes; ++i) {
            ...
            // 优先选择720p模式（1280x720）
            if (m_connector->modes[i].hdisplay == 1280 && m_connector->modes[i].vdisplay == 720) {
                m_selected_mode_index = i;
                printf("[DisplayDevice::init] ✓ Found 720p mode, will use mode[%d]\n", i);
            }
        }
...
int DisplayDevice::displayFrameYUV(const void* yuv_data, uint32_t width, uint32_t height) {
    ...
    // YUV转RGB
    printf("[DisplayDevice::displayFrameYUV] Converting YUV to RGB: %ux%u\n", width, height);
    ...
    if (!m_crtc_set) {
        ...
        // 检查分辨率是否匹配
        if (width != m_connector->modes[m_selected_mode_index].hdisplay || 
            height != m_connector->modes[m_selected_mode_index].vdisplay) {
            fprintf(stderr, "[DisplayDevice::displayFrameYUV] ⚠️  Resolution mismatch!\n");
            fprintf(stderr, "[DisplayDevice::displayFrameYUV]   FB: %ux%u, Mode: %ux%u\n",
                    width, height,
                    m_connector->modes[m_selected_mode_index].hdisplay,
                    m_connector->modes[m_selected_mode_index].vdisplay);
        }
...
void DisplayDevice::yuvToRgb(const uint8_t* yuv, uint8_t* rgb, uint32_t width, uint32_t height) {
    ...
    for (uint32_t y = 0; y < height; ++y) {
        for (uint32_t x = 0; x < width; ++x) {
            ...
            int uv_idx = ((y / 2) * (width / 2) + (x / 2)) * 2;
            int u_val = uv_plane[uv_idx] - 128;
            int v_val = uv_plane[uv_idx + 1] - 128;

            // YUV to RGB转换
            int r = y_val + (int)(1.402 * v_val);
            int g = y_val - (int)(0.344 * u_val + 0.714 * v_val);
            int b = y_val + (int)(1.772 * u_val);
